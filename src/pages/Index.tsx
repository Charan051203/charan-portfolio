...
const jokes = ["Why do programmers prefer dark mode? Because light attracts bugs.", "I told my computer I needed a break, and now it won't stop sending me vacation ads.", "Why don't programmers like nature? It has too many bugs and no debugging tool.", "How many programmers does it take to change a light bulb? None, that's a hardware problem.", "What's a programmer's favorite place? Function junction.", "Why did the developer go broke? Because they lost their domain.", "What do you call a programmer from Finland? Nerdic.", "I would tell you a UDP joke, but you might not get it.", "Why was the JavaScript developer sad? Because they didn't know how to 'null' their feelings.", "The programmer got stuck in the shower because the instructions on the shampoo bottle said: Lather, Rinse, Repeat."];

// Gaming jokes - add some gaming themed humor
const gamingJokes = ["I would tell you a joke about lag, but you wouldn't get it until later.", "Why don't developers play video games? Because they prefer to debug than to play.", "What do you call a game developer's favorite coffee? Java.", "Why was the gamer always broke? Too many micro-transactions.", "How many gamers does it take to change a light bulb? None, they'll do it when the next patch comes out."];
const Index: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const isSlowConnection = 'connection' in navigator && ((navigator as any).connection?.effectiveType === '2g' || (navigator as any).connection?.saveData);
    if (!isSlowConnection) {
  ...
  return <div className="relative">
      ...
      {showSidebar && <div className="fixed left-4 sm:left-6 bottom-1/2 transform translate-y-1/2 flex-col gap-4 sm:gap-5 z-30 hidden lg:flex">
          {[{
        ...
        href: "#",
        label: "Gaming"
      }].map((item, i) => <motion.a key={i} href={item.href} target="_blank" rel="noopener noreferrer" className="w-9 h-9 sm:w-10 sm:h-10 rounded-full glassmorphism flex items-center justify-center text-foreground hover:text-primary hover:border-primary border border-border/50 transition-all" initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        boxShadow: '0 0 15px hsla(var(--primary), 0.4)'
          ...
          <motion.div className="w-px h-16 sm:h-24 bg-border/50 mx-auto mt-2" initial={{
        height: 0
      }} animate={{
      ...
      <motion.a href="#home" className="fixed bottom-4 right-4 sm:bottom-6 sm:right-5 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-primary flex items-center justify-center hover:bg-primary/90 transition-all z-30 border-2 border-primary/30 back-to-top" initial={{
      opacity: 0,
      scale: 0.8
    }} animate={{
