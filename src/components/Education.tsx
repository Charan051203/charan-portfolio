...
const Education: React.FC = () => {
  ...
  return <section id="education" className="py-24 relative">
      ...
      <div className="container mx-auto px-6">
        ...
        {/* Desktop Timeline */}
        <motion.div className="max-w-3xl mx-auto relative hidden md:block" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{
        once: true,
        margin: "-100px"
      }}>
          {/* Timeline line */}
        ...
        <motion.div className="md:hidden space-y-8" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{
        once: true,
        margin: "-100px"
      }}>
          {/* Timeline line - fixed positioning */}
          <div className="absolute left-8 top-[22rem] bottom-24 w-0.5 z-0 bg-slate-800 px-px mx-[147px] my-[62px]" />
          
          {timelineItems.map((item, index) => <motion.div key={index} variants={itemVariants} className="relative pl-16 ml-2 my-[44px] mx-0 px-px">
              {/* Timeline dot with pulse effect */}
              <motion.div className="absolute left-7 top-7 transform -translate-x-1/2 w-4 h-4 rounded-full bg-primary z-10" initial={{
            scale: 0
          }} whileInView={{
            scale: 1
          }} transition={{
            ...
            once: true
              ...
              {/* Mobile Content */}
              <motion.div className="bg-card border border-border rounded-xl p-6 shadow-lg glassmorphism relative z-10" whileHover={{
            x: 5,
            boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)"
          }} transition={{
            duration: 0.3
              ...
              {/* Connector line with animation */}
              <motion.div className="absolute top-7 left-7 w-8 h-0.5 bg-primary/40 z-5" initial={{
            width: 0
          }} whileInView={{
            width: 32
          }} transition={{
